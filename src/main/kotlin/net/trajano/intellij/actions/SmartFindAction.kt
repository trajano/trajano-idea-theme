...
class SmartFindAction : AnAction() {
        ...
        dispatcher = KeyEventDispatcher { event ->
            when (event.id) {
                KeyEvent.KEY_RELEASED -> {
                    println("Release ${KeyEvent.getKeyText(event.keyCode)}")
                    when (event.keyCode) {
                ...
                KeyEvent.KEY_PRESSED -> {
                    ...
                    println("Pressed ${KeyEvent.getKeyText(event.keyCode)}")
                    if (!ctrlWasReleased) {
            ...
            if (ctrlWasReleased) {
                ...
                println("ctrlWasReleased isTriggered=$isTriggered")
                cleanupDispatcher()
                if (!isTriggered) {
                    triggerAction(IdeActions.ACTION_FIND, e)
                }
                // running is stopped once the Ctrl key is released
                isRunning = false
                return@KeyEventDispatcher true
            }
            if (isTriggered) return@KeyEventDispatcher true
            if (!(shortTimer?.isRunning ?: false)) {
                shortTimer = Timer(250) {
                    if (charWasReleased && !secondaryCharWasPressedWhileCtrlWasHeld) {
                        println("charWasReleased and not secondaryCharWasPressedWhileCtrlWasHeld and 250ms hit")
                    ...
                    longTimer = Timer(250) {
                        if (!charWasReleased && !secondaryCharWasPressedWhileCtrlWasHeld) {
                            println("not charWasReleased and not secondaryCharWasPressedWhileCtrlWasHeld and 500ms hit")
                            triggerAction(IdeActions.ACTION_FIND_IN_PATH, e)
                            return@Timer
                        }
                    }
            ...
            if (secondaryCharWasPressedWhileCtrlWasHeld) {
                when (event.keyCode) {
                    KeyEvent.VK_R -> triggerAction(IdeActions.ACTION_REPLACE, e)
                    KeyEvent.VK_F -> triggerAction(IdeActions.ACTION_FIND_IN_PATH, e)
